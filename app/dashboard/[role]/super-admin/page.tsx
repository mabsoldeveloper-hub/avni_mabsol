...
import { FaSync, FaUserCheck, FaChevronDown } from "react-icons/fa";
        ...
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
              Admin Clearance & Approvals
            </h1>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10.5px] font-bold bg-amber-50 text-amber-800 border border-amber-200/80">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
              {stats.pending} Awaiting Review
            </span>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Review incoming enterprise applications, set validity periods, and configure module permissions
          </p>
        </div>
      ...
      fallback={
        <div className="min-h-[400px] flex items-center justify-center">
          <div className="flex items-center gap-2 text-slate-500 text-sm font-semibold">
            ...
            Loading super admin approvals...
          </div>
        </div>
      }
    >
      <ApproveAdminsContent />
    </Suspense>
  );
}
