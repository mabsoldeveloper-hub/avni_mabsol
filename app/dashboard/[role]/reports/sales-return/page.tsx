...
export default function SalesReturnReportPage() {
    ...
    // ── Debit Note State ──
    const [dnRows, setDnRows] = useState<any[]>([]);
    const [dnExpandedRows, setDnExpandedRows] = useState<Set<string>>(new Set());
    ...
    const [dnSummary, setDnSummary] = useState({
        totalCount: 0,
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.success) {
                    ...
                    setDnTotalPages(data.pagination?.totalPages || 1);
                }
            }
        } catch (err) {
            console.error("Failed to load debit note report:", err);
        } finally {
            setDnLoading(false);
        }
    }, [dnPage, appliedFilters, selectedCompany, selectedFY]);

    useEffect(() => { fetchDebitNotes(); }, [fetchDebitNotes]);

    const toggleDnRow = (id: string) => {
        const updated = new Set(dnExpandedRows);
        if (updated.has(id)) updated.delete(id);
        else updated.add(id);
        setDnExpandedRows(updated);
    };
    ...
    return (
        <div className="p-3 sm:p-6 space-y-4 max-w-[1600px] mx-auto print:p-0">
            ...
            {/* Debit Note KPI Cards */}
            <div className="mt-2 flex items-center gap-2 mb-3">
                <div className="h-px flex-1 bg-amber-200/60 dark:bg-amber-900/40" />
                <span className="flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                    <FaExchangeAlt size={11} /> Debit Note Analytics (Purchase Returns to Supplier)
                </span>
                <div className="h-px flex-1 bg-amber-200/60 dark:bg-amber-900/40" />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 print:grid-cols-5 print:gap-2">
                <div className="bg-gradient-to-br from-amber-50 to-white dark:from-amber-950/40 dark:to-slate-900 p-4 rounded-2xl border border-amber-200/80 dark:border-amber-900/50 shadow-xs flex items-center justify-between">
                    <div>
                        ...
                        <span className="text-xl font-bold text-slate-900 dark:text-white font-mono">₹{dnSummary.totalDebitNoteAmount.toLocaleString("en-IN")}</span>
                        <span className="text-[10px] text-slate-400 block mt-0.5">{dnSummary.totalCount} Debit Note Vouchers</span>
                    </div>
                    <div className="w-10 h-10 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center flex-shrink-0">
                ...
                <div className="bg-gradient-to-br from-orange-50 to-white dark:from-orange-950/40 dark:to-slate-900 p-4 rounded-2xl border border-orange-200/80 dark:border-orange-900/50 shadow-xs flex items-center justify-between">
                    <div>
                        ...
                        <span className="text-xl font-bold text-slate-900 dark:text-white font-mono">₹{dnSummary.totalTaxableAmount.toLocaleString("en-IN")}</span>
                ...
                <div className="bg-gradient-to-br from-yellow-50 to-white dark:from-yellow-950/40 dark:to-slate-900 p-4 rounded-2xl border border-yellow-200/80 dark:border-yellow-900/50 shadow-xs flex items-center justify-between">
                    <div>
                        <span className="text-[11px] font-semibold text-yellow-600 dark:text-yellow-400 uppercase tracking-wider block">GST Input Claimed</span>
                        <span className="text-xl font-bold text-slate-900 dark:text-white font-mono">₹{dnSummary.totalTaxAmount.toLocaleString("en-IN")}</span>
                        <span className="text-[10px] text-slate-400 block mt-0.5">ITC Adjustment on Returns</span>
                    </div>
                    <div className="w-10 h-10 rounded-xl bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 flex items-center justify-center flex-shrink-0">
                        <FaBoxes size={18} />
                    </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-lime-50 to-white dark:from-lime-950/40 dark:to-slate-900 p-4 rounded-2xl border border-lime-200/80 dark:border-lime-900/50 shadow-xs flex items-center justify-between">
                    <div>
                        ...
                        <span className="text-xl font-bold text-slate-900 dark:text-white font-mono">{dnSummary.totalItemsQty} Units</span>
                ...
                <div className="bg-gradient-to-br from-teal-50 to-white dark:from-teal-950/40 dark:to-slate-900 p-4 rounded-2xl border border-teal-200/80 dark:border-teal-900/50 shadow-xs flex items-center justify-between">
                    <div>
                        ...
                        <span className="text-xl font-bold text-slate-900 dark:text-white font-mono">₹{dnSummary.avgDebitNoteAmount.toLocaleString("en-IN")}</span>
            ...
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-4 sm:p-5 rounded-2xl border border-amber-200/60 dark:border-amber-900/40 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-amber-100 dark:border-amber-900/40 pb-3 print:hidden">
                    <span className="text-xs font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
                        <FaExchangeAlt /> Debit Note Vouchers List ({dnSummary.totalCount})
                    </span>
                    <button onClick={fetchDebitNotes} className="p-1.5 rounded-lg text-slate-400 hover:text-amber-600">
                        <FaSync className={dnLoading ? "animate-spin text-amber-500" : ""} size={13} />
                    </button>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-200/70 dark:divide-slate-800">
                            {dnLoading ? (
                                <tr><td colSpan={12} className="py-10 text-center text-slate-400">
                                    <FaSync size={16} className="animate-spin text-amber-500 mx-auto mb-2" />
                                    Loading Debit Notes...
                                </td></tr>
                            ) : dnRows.length === 0 ? (
                                <tr><td colSpan={12} className="py-10 text-center text-slate-400">
                                ...
                                dnRows.map((r) => {
                                    const isExpanded = dnExpandedRows.has(r.id);
                                    return (
                                        <React.Fragment key={r.id}>
                                            <tr className="hover:bg-amber-50/50 dark:hover:bg-amber-950/10 transition cursor-pointer" onClick={() => toggleDnRow(r.id)}>
                                                <td className="p-2.5 text-center text-slate-400">
                                                    {isExpanded ? <FaChevronUp size={10} /> : <FaChevronDown size={10} />}
                                                </td>
                                                <td className="p-2.5 font-mono font-bold text-amber-700 dark:text-amber-400">{r.vcn}</td>
                                                <td className="p-2.5 font-mono text-slate-600 dark:text-slate-400">{r.date}</td>
                                                <td className="p-2.5 font-semibold text-slate-900 dark:text-white">
                                                    {r.partyName}
                                                    <div className="flex items-center gap-2 text-[10px] text-slate-400 font-normal mt-0.5">
                                                        <span className="font-mono">#{r.partyCode}</span>
                                                ...
                                                <td className="p-2.5 text-slate-600 dark:text-slate-400">{r.salesman || "—"}</td>
                                                <td className="p-2.5">
                                                    {r.remarks ? (
                                                ...
                                                <td className="p-2.5 text-right font-mono text-slate-600 dark:text-slate-400">₹{r.taxableAmount.toLocaleString("en-IN")}</td>
                                                <td className="p-2.5 text-right font-mono text-slate-600 dark:text-slate-400">₹{r.taxAmount.toLocaleString("en-IN")}</td>
                                                <td className="p-2.5 text-right font-mono font-bold text-amber-700 dark:text-amber-400">₹{r.finalAmount.toLocaleString("en-IN")}</td>
                                            </tr>
                                            {isExpanded && (
                                                        ...
                                                        <div className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-amber-200 dark:border-amber-900/40 space-y-3 shadow-inner">
                                                            <div className="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300 pb-2 border-b border-amber-100 dark:border-amber-900/30">
                                                                <span className="flex items-center gap-2">
                                                                    ...
                                                                    Returned Products — Debit Note Breakdown ({r.items.length} Products, {r.totalQty} Qty)
                                                                </span>
                                                                {r.phone && <span><FaPhoneAlt size={10} className="inline mr-1" />{r.phone}</span>}
                                                            </div>
                                                            {r.items.length === 0 ? (
                                                                <p className="text-xs text-slate-400">No line item details available for this debit note.</p>
                                                            ) : (
                                                                <table className="w-full text-left text-xs border-collapse">
                ...
                {/* DN Pagination */}
                <div className="flex items-center justify-between pt-2 text-xs print:hidden">
                    <span className="text-slate-500">Page {dnPage} of {dnTotalPages} ({dnSummary.totalCount} total debit notes)</span>
                    <div className="flex items-center gap-1">
                        <button disabled={dnPage <= 1 || dnLoading} onClick={() => setDnPage((p) => p - 1)}
                            className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 disabled:opacity-40">
                            <FaChevronLeft size={10} />
                        </button>
                        <button disabled={dnPage >= dnTotalPages || dnLoading} onClick={() => setDnPage((p) => p + 1)}
                            className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 disabled:opacity-40">
                            <FaChevronRight size={10} />
                        </button>
                    </div>
