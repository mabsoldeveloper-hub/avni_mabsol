...
export default function EditUserPage() {
  ...
  useEffect(() => {
    Promise.all([loadCompanies(), loadRoles(), loadUsers(), loadUser()]).finally(() => {
  ...
  const loadUser = async () => {
    try {
      ...
      setForm({
        ...
        roleType: data.roleType || "MR",
        assignedAreaNames: Array.isArray(data.assignedAreaNames) ? data.assignedAreaNames : (data.assignedAreaName ? [data.assignedAreaName] : []),
  ...
  if (initialLoading) {
    return (
      <div className="py-20 flex flex-col items-center justify-center text-slate-400 gap-3">
        ...
        <span className="text-xs font-semibold">Loading user details...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-400 mb-1">
            ...
            <span>/</span>
            <span className="text-slate-700 dark:text-slate-200 font-semibold">Edit User: {form.name}</span>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={updateUser}
            disabled={loading}
            className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 shadow-lg shadow-purple-600/25 transition-all hover:scale-102 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="rounded-2xl p-5 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col items-center text-center">
            ...
            <h4 className="text-base font-bold text-slate-800 dark:text-slate-100">{form.name || "User Profile"}</h4>
            <p className="text-xs text-slate-400 mt-0.5">{form.email}</p>

            <div className="flex flex-wrap items-center justify-center gap-1.5 mt-3">
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-purple-50 dark:bg-purple-950 text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-800">
                {selectedRole?.roleName || form.roleType || "User"}
              </span>
        ...
        <div className="lg:col-span-8 flex flex-col gap-4">
            ...
            <div className="rounded-2xl p-5 sm:p-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col gap-5 animate-in fade-in duration-300">
              <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  <FaShieldAlt className="text-purple-600" /> Account Credentials &amp; Role Assignment
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">Select from available system roles.</p>
              </div>
                    ...
                    return (
                      <div
                        ...
                        className={`p-3.5 rounded-xl border cursor-pointer transition-all flex flex-col justify-between ${
                          isSelected
                            ? "bg-purple-50/90 dark:bg-purple-950/60 border-purple-500 shadow-md ring-2 ring-purple-500/20"
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {isSelected && (
                              <FaCheckCircle className="text-purple-600 shrink-0" size={14} />
                            )}
                          </div>
                          {r.description && (
              ...
              <div className="mt-2 flex justify-end">
                <button
                  type="button"
                  onClick={() => setActiveSection("organization")}
                  className="px-4 py-2 rounded-xl bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-400 hover:bg-purple-100 text-xs font-bold transition-all"
                >
          ...
          {activeSection === "organization" && (
            <div className="rounded-2xl p-5 sm:p-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col gap-4 animate-in fade-in duration-300">
              <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  <FaBuilding className="text-purple-600" /> Organization &amp; GST-Powered Territory Setup
                </h3>
          ...
          {activeSection === "hierarchy" && (
            <div className="rounded-2xl p-5 sm:p-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col gap-4 animate-in fade-in duration-300">
              <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  <FaSitemap className="text-purple-600" /> Sales Hierarchy &amp; Management Tree
                </h3>
          ...
          {activeSection === "personal" && (
            <div className="rounded-2xl p-5 sm:p-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col gap-4 animate-in fade-in duration-300">
              <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
                <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                  <FaIdCard className="text-purple-600" /> Personal Details &amp; Permanent Address
                </h3>
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={updateUser}
                  disabled={loading}
                  className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 shadow-lg shadow-purple-600/30 transition-all hover:scale-105"
                >
                  {loading ? "Updating..." : "✓ Save & Update User"}
                </button>
              </div>
            </div>
          )}
